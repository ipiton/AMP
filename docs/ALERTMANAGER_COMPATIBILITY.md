...
|----------|--------------|---------------|---------|-------|
| `GET /api/v2/status` | ✅ | ✅ **ACTIVE** | 🟢 | Runtime-backed status shape with `cluster`, `versionInfo`, `config`, `uptime`; single-node reports `cluster.status=disabled`; `uptime` uses upstream-like millisecond precision timestamp format |

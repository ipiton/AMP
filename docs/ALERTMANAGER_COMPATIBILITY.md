...
| `POST /api/v2/alerts` | ✅ | ✅ **ACTIVE** | 🟡 | Ingest + dedup + resolve semantics; error contracts closer to upstream (`400` `{code,message}` for parse/time errors, `422` `{code:602}` for missing labels, `422` `{code:601}` for invalid `generatorURL`, `400` JSON string for empty labels); date-only ingest timestamps (`YYYY-MM-DD`) accepted for `startsAt`/`endsAt`; no full upstream routing tree parity |

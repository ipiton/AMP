...
|----------|--------------|---------------|---------|-------|
| `GET /api/v2/status` | ✅ | ✅ **ACTIVE** | 🟢 | Runtime-backed status shape with `cluster`, `versionInfo`, `config`, `uptime`; cluster mode is upstream-like configurable (`AMP_CLUSTER_LISTEN_ADDRESS=` -> disabled, otherwise ready self-peer shape); `uptime` uses upstream-like millisecond precision timestamp format |

...
- [x] **FU-RECEIVERS-INTEGRATION** _(closed by AMP-PARITY-WAVE6-EPIC, 2026-08-19; slice 1 + 2, three review rounds)_ — receivers: integration auto-provisioning. Every `webhook`/`slack`/`pagerduty`/`telegram`/`email` integration block now provisions an in-memory `core.PublishingTarget` (`cfg:<receiver>/<type><idx>`, receiver-scoped by construction, rebuilt atomically on reload) that merges into the SAME discovery view as K8s-Secret targets, with the publisher layer untouched; `global:` endpoint fallbacks (`slack_api_url`/`pagerduty_url`/`telegram_api_url`/`smtp_*`) and per-integration `send_resolved` are honoured; blackhole receivers (zero integrations, or only unsupported ones) load and drop upstream-style; the lite profile delivers config-only with no K8s at all. Review rounds also fixed a pre-existing cross-receiver fan-out on the non-grouped publish path (default `grouping.enabled: false`), the PagerDuty `/v2/events` → `/v2/enqueue` endpoint bug that made every PagerDuty notification 404, and duplicate-receiver-name shadowing. Residual: per-field fidelity, tracked below. ~5-7d
- [ ] **FU-INTEGRATION-FIELD-FIDELITY** _(opened by AMP-PARITY-WAVE6-EPIC, 2026-08-19; `*_file` credential variants carved out and closed separately as `FU-FILE-SECRETS`, wave 7 track B, 2026-08-19)_ — per-integration FIELD fidelity for auto-provisioned receivers: Slack `channel`/`username`/`icon_*`/`title`/`title_link`/`pretext`/`text`/`fields`/`actions`/`color`/`short_fields`, PagerDuty `severity`/`class`/`component`/`group`/`description`/`details`, Telegram `parse_mode`/`message`, webhook `http_method`/`max_alerts`, `email_configs[].headers` and upstream's per-`email_config` `smarthost`/`auth_*`/`require_tls`. All parse and validate today; none reach the wire, because each enhanced publisher renders through the shared `AlertFormatter` and reads nothing off the target beyond the credentials the factory extracts. Needs a per-target message-config path through the publishers (i.e. it crosses the epic's "publisher layer unchanged" boundary, hence deferred) plus notification-template integration for the templated fields. Also covers per-integration `http_config` (proxy/TLS/auth/redirects). ~3-5d
- [x] **FU-FILE-SECRETS** _(opened + closed by wave 7 track B, 2026-08-19; fix round same day: review REJECT — added the undisclosed `service_key_file` gap, pinned the reload-rotation test at the target level, added whitespace-only-file coverage)_ — `*_file` secret variants for integration configs: `url_file` (webhook), `routing_key_file` + `service_key_file` (pagerduty — both the current and the legacy routing-key field get a twin, matching upstream's `notify/pagerduty/config.go`), `api_url_file` (slack), `bot_token_file` (telegram), `slack_api_url_file` / `smtp_auth_password_file` (global) — the Kubernetes-Secret-volume shape upstream Alertmanager accepts for every credential field. `internal/infrastructure/routing.resolveFileSecrets` (new, `file_secrets.go`) enforces upstream's mutual-exclusion rule (inline XOR file, both = load error naming the field; unreadable file = load error naming the path) and resolves the file's content — trimmed of trailing newline/whitespace — into the SAME inline field `resolveGlobalFallbacks`, `applyDefaults`, validation and `business/publishing.BuildConfigTargets` already read, so the target builder needed zero changes. Re-read on every config load/reload (not lazily per-notification — documented divergence from upstream, requires `/-/reload`/SIGHUP for rotation) — pinned by a target-level test that rotates a file between two independent `Parse`+`BuildConfigTargets` calls and asserts the second reflects the new content. Status-API redaction (`internal/application/handlers/status_config.go`) taught a `fileSecretRefKeys` exception list so the `*_file` field's path stays visible (several of the new key names would otherwise trip an unrelated substring by coincidence — `routing_key_file`/`service_key_file` contain `routing_key`/`service_key`, `bot_token_file` contains `token` — and get redacted right along with the content they name) while the resolved secret stays redacted regardless of source. `global.smtp_auth_secret_file` (upstream's CRAM-MD5 secret) deliberately not added — AMP's SMTP publisher has no `smtp_auth_secret` field to pair it with; this remains the ONLY undelivered `*_file` field, now explicitly the sole exclusion. ~1d + ~0.5d fix round
- [ ] **FU-INTEGRATION-FIELD-FIDELITY** _(opened by AMP-PARITY-WAVE6-EPIC, 2026-08-19)_ — per-integration FIELD fidelity for auto-provisioned receivers: Slack `channel`/`username`/`icon_*`/`title`/`title_link`/`pretext`/`text`/`fields`/`actions`/`color`/`short_fields`, PagerDuty `severity`/`class`/`component`/`group`/`description`/`details`, Telegram `parse_mode`/`message`, webhook `http_method`/`max_alerts`, `email_configs[].headers` and upstream's per-`email_config` `smarthost`/`auth_*`/`require_tls`. All parse and validate today; none reach the wire, because each enhanced publisher renders through the shared `AlertFormatter` and reads nothing off the target beyond the credentials the factory extracts. Needs a per-target message-config path through the publishers (i.e. it crosses the epic's "publisher layer unchanged" boundary, hence deferred) plus notification-template integration for the templated fields. Also covers `*_file` credential variants for the integration credentials themselves (`api_key_file`, `bot_token_file`, `routing_key_file`, …), which the routing schema does not model at all. Per-integration `http_config` is NO LONGER part of this item — delivered by `FU-HTTP-CONFIG` (wave 7 track C), including its own `password_file`/`credentials_file`. ~3-5d
- [x] **FU-HTTP-CONFIG** _(opened + closed by AMP-PARITY-WAVE7 track C, 2026-08-19)_ — per-integration `http_config` for publisher HTTP clients. `webhook_configs`/`slack_configs`/`pagerduty_configs`/`telegram_configs` (and `global.http_config` as fallback) now deliver `proxy_url`, `tls_config` (`ca_file`/`cert_file`/`key_file`/`server_name`/`insecure_skip_verify`), `basic_auth` (+`password_file`), `authorization` (+`credentials_file`, `type` defaults to `Bearer`) and `follow_redirects`. New `core.HTTPClientConfig` travels config→`PublishingTarget`→`CreatePublisherForTarget`; K8s-Secret targets get it free via the same JSON blob. `applyHTTPClientConfig` layers onto the client each publisher would have built, so existing pool/timeout/TLS-floor tuning survives and a target without `http_config` is byte-identical to before. Global fallback is WHOLESALE, never deep-merged (upstream `Config.UnmarshalYAML` semantics, mirrored including the surprising part). Every per-type client cache key gained the config fingerprint (third occurrence of the telegram/pagerduty cache-key defect class); the new `httpClientMap` is keyed by (publisher shape, fingerprint) so shapes with different base tunings cannot share a client. Fails CLOSED per target on unreadable/unusable TLS or credential files, `basic_auth`+`authorization` together, or an unroutable `proxy_url` — logged `ERROR` naming target + path, that target skipped, others keep delivering. `CreateBasicPublisherForTarget` was added so the queue's webhook path (deliberately on the basic publisher, to avoid switching on `WebhookValidator`) honours it too — webhooks being the likeliest users of a corp proxy or mTLS. **Fix round (security review, same day)**: C1 — the four credential-missing `createEnhanced*Publisher` fallbacks returned bare `NewXPublisher()` and therefore dropped `http_config` wholesale (unauthenticated payloads; a pinned `ca_file` silently swapped for the system trust store with `err == nil`), now routed through `CreateBasicPublisherForTarget`; I1 — oauth2 provenance split (see ADR-008); M1 — fingerprint length-prefixed instead of NUL-delimited (a NUL-bearing `proxy_url` could impersonate a `basic_auth` block and collapse two configs onto one client); M7/M8 — `core.HTTPClientConfig.Validate()` runs at config build AND K8s-Secret discovery, so structural mistakes skip the target once instead of failing every job into the DLQ; M10 — `Normalize()` shared by both sources; M2 — `server_name` isolation test; plus relative `*_file` path rebasing onto the config directory (upstream `resolveFilepaths` parity, `internal/config.resolveHTTPConfigFilepaths`). ~2d
- [ ] **FU-HEALTH-HTTP-CONFIG** _(opened by AMP-PARITY-WAVE7 track C security review I2, 2026-08-19)_ — the publishing health monitor (`internal/business/publishing/health_impl.go`, `NewHealthMonitor`) probes EVERY discovered target with ONE process-wide `*http.Client`: its own transport, its own `TLSSkipVerify`, `Proxy` unset. A target reachable only via `http_config.proxy_url`, or requiring a client certificate, or whose endpoint 401s without `basic_auth`/`authorization`, fails its probe and reports `degraded`/`unhealthy` while DELIVERING correctly. Not a delivery bug today — `filterHealthyTargets` lives on `DefaultParallelPublisher`, which has no non-test constructor, and `PublishingQueue.processJob` does not consult the monitor — so the blast radius is an observability false negative that nonetheless feeds `NewHealthMetricsCollector` and therefore alerting, and becomes a real delivery bug the moment anything wires `filterHealthyTargets` up. Fix by reusing the factory's per-target client cache (`PublisherFactory.perTargetHTTPClient`) for probes, which means injecting the factory into `NewHealthMonitor` and its callers — a constructor change across a layer boundary, hence ledgered rather than bundled. Documented in the code and in `docs/ALERTMANAGER_COMPATIBILITY.md`. ~0.5-1d
- [ ] **FU-HTTP-CONNECT-PROXY-COVERAGE** _(opened by AMP-PARITY-WAVE7 track C security review M3, 2026-08-19)_ — AMP's `http_config.proxy_url` test asserts the plain-HTTP proxy path (absolute-URI request line, with a no-proxy control). The brief permitted "CONNECT **or** plain-HTTP", so this is in spec, but nearly every real webhook is `https://` — i.e. the CONNECT tunnel is the common path in practice and the one with no AMP-side coverage. It relies entirely on `net/http`'s own CONNECT handling (AMP only sets `Transport.Proxy`), so risk is low; add a CONNECT-capable `httptest` proxy to close the gap. Noted in the compatibility doc meanwhile. ~0.25d
- [ ] **FU-HTTP-OAUTH2** _(opened by AMP-PARITY-WAVE7 track C, 2026-08-19)_ — `http_config.oauth2` (`client_id`/`client_secret`/`client_secret_file`/`token_url`/`scopes`/`endpoint_params`). Modelled and reported today, never applied: an `oauth2:` block produces one loud `WARN` naming the receiver, integration kind and index, and requests go out without OAuth2 credentials (the endpoint's 401s are the other half of the signal). Deliberately out of `FU-HTTP-CONFIG`'s scope because it is not a field mapping — it needs a token-endpoint client, a refresh loop with its own failure/retry/backoff semantics and its own cache invalidation when a token expires mid-flight, plus a decision on whether to take a dependency (`golang.org/x/oauth2`) or hand-roll it. Also needs the client cache-key fingerprint extended to cover the oauth2 fields. ~1-2d
- [ ] **FU-HTTP-FILE-ROTATION** _(opened by AMP-PARITY-WAVE7 track C, 2026-08-19)_ — `http_config`'s TLS and credential FILE contents are captured at client construction and then cached per distinct config, so rotating a client certificate, CA bundle or `password_file` on disk does not affect an already-built client until a process restart (or a config reload that changes the config itself). Upstream re-reads per handshake/request via `prometheus/common/config`. Fix by moving the reads behind `tls.Config.GetClientCertificate`/`GetConfigForClient` and a re-reading credential provider in `authRoundTripper`, with an mtime or content check so a rotated file is picked up without rebuilding the connection pool. Matters most for cert-manager-style short-lived certificates. ~1d
- [x] **FU-INTEGRATION-FIELD-FIDELITY** _(closed by TEMPLATES-EPIC slice 2, 2026-08-19)_ — the PRESENTATION half is delivered: Slack `title`/`title_link`/`pretext`/`text`/`color`/`username`/`icon_emoji`/`icon_url`/`channel`, PagerDuty `description`/`severity`/`client`/`client_url`/`details.*`, Telegram `message`, Email `subject`/`html`/`text`/`headers.*` now render through the notification template engine and reach the wire (`core.PublishingTarget.Templates` ← `business/publishing/config_target_templates.go` → `infrastructure/publishing/template_formatter.go`), with upstream's own defaults materialized so an untouched upstream config produces upstream's output. Telegram `parse_mode` is delivered as config (not templated, as upstream). Done WITHOUT touching a wire sender: a per-target formatter decorator, plus additive omitempty fields on Slack's message/attachment model that previously had nowhere to carry a title. Residual, re-scoped below as FU-INTEGRATION-TRANSPORT-FIDELITY: per-integration `http_config`, `*_file` credential variants, webhook `http_method`/`max_alerts`, Slack `fields`/`actions`/`short_fields`, PagerDuty `class`/`component`/`group`/`images`, per-`email_config` SMTP. ~3-5d
- [ ] **FU-INTEGRATION-TRANSPORT-FIDELITY** _(re-scoped from FU-INTEGRATION-FIELD-FIDELITY on its closure, 2026-08-19)_ — what is left after the presentation fields shipped, all of it transport or structural rather than templated: per-integration `http_config` (proxy/TLS/bearer/basic/`follow_redirects`) which every publisher ignores in favour of its own client; `*_file` credential variants (`api_key_file`, `bot_token_file`, `routing_key_file`, `auth_password_file`, …) which `internal/infrastructure/routing` does not model at all; webhook `http_method`/`max_alerts`; Slack `fields`/`actions`/`short_fields` (Block Kit structures the wire model does not carry); PagerDuty `class`/`component`/`group`/`images`; per-`email_config` `smarthost`/`auth_*`/`require_tls` (only `global.smtp_*` reaches the dialer, so two receivers cannot use different SMTP servers). Note while doing `http_config`: upstream's `resolveFilepaths` rebases those FILE paths against the config file's directory, exactly as `templates:` globs now are — AMP will need the same treatment there. ~3-4d
- [x] **TEMPLATES-EPIC** _(closed 2026-08-19; slice 1 + slice 2, one review round each)_ — upstream-compatible notification templates, the last functional drop-in gap. Slice 1: `internal/business/templating` — upstream's `Data` model, all 25 `DefaultFuncs`, byte-identical copies of v0.34.0's `default.tmpl`/`email.tmpl` (Apache-2.0 notices retained), `templates:` glob loading resolved against the CONFIG FILE's directory (upstream's `resolveFilepaths`) with later-definition-wins overrides, per-file parse errors naming file+line, atomic reload swap, and execution guards upstream lacks (timeout + output cap + panic recovery, operator-tunable via `publishing.templates.*`). Slice 2: per-integration presentation fields rendered onto the wire through a per-target formatter decorator, with the fallback contract that a failed render delivers AMP's fixed formatter output and increments `alert_history_publishing_template_fallbacks_total{integration,reason}` rather than dropping the notification. Goldens generated by upstream's own template package. Closed `FU-INTEGRATION-FIELD-FIDELITY` in the same pass. ~5-7d
- [ ] **FU-UNWIRED-PUBLISH-SURFACES** _(opened by FU-RECEIVERS-INTEGRATION slice-1 re-review, 2026-08-19)_ — two unwired publishing surfaces that would leak or misdeliver if wired as-is, now that `receivers:` auto-provisions one target per integration: (1) `DefaultParallelPublisher.PublishToAll` resolves targets itself with NO receiver filtering (no non-test constructor call anywhere — same latent cross-receiver fan-out class as `PublishingCoordinator.PublishToAll`, which is documented do-not-use); (2) `PublishingHandlers.ListTargets`/`GetTarget` return `URL` + `Headers` verbatim over HTTP (`NewPublishingHandlers` has no non-test caller, no route registers `/api/v1/publishing/targets`), which would expose `bot_token`/`smtp_password`/webhook URLs for BOTH target sources. Either wire them with receiver scoping + the status-API redaction treatment, or delete them. ~0.5-1d
...
- [x] **FU-FLAKE-EVENTKEYCACHE-GOROUTINE-COUNT** _(opened by AMP-PARITY-WAVE6-EPIC slice-2 review, 2026-08-19)_ — `internal/infrastructure/publishing.TestEventKeyCache_StopStopsCleanupWorker` asserts on `runtime.NumGoroutine()` deltas against a baseline taken at test start, so ANY neighbouring test whose teardown goroutines are still unwinding makes it fail. Observed failing once in a full-package run, then green on 3 consecutive full-package runs and 5 consecutive isolated runs; pre-existing (the file is untouched by wave 6) and the same family as the goroutine-count issues recorded under `FU-WAVE3-RELIABILITY`. Fix by asserting the cache's own worker exited (a done channel or `sync.WaitGroup` the cache exposes for tests) instead of a process-wide goroutine count. ~0.25d _(closed 2026-08-19: worker exit signalled via workerDone channel, test waits deterministically; -count=20 -race stable)_
- [ ] **FU-E2E-HTTP-INGEST-DELIVERY** _(opened by AMP-PARITY-WAVE6-EPIC slice-2 review, 2026-08-19)_ — the config-only delivery e2e (`go-app/internal/application/config_only_delivery_e2e_test.go`) enters at `AddAlertToGroup`, not `POST /api/v2/alerts`, so the handler → AlertProcessor → route-evaluator hop of the "an untouched upstream config delivers" claim rests on unit tests only. Add an HTTP-in/HTTP-out case to the `deploy/e2e-ha` harness (real storage/Redis already there): ingest → route evaluation → group_wait → webhook receipt, plus a receiver-scoping assertion (alerts for receiver A must not reach receiver B's endpoint). ~0.5d

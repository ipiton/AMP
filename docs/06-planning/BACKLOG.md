...
- [ ] **FU-FLAKE-EVENTKEYCACHE-GOROUTINE-COUNT** _(opened by AMP-PARITY-WAVE6-EPIC slice-2 review, 2026-08-19)_ — `internal/infrastructure/publishing.TestEventKeyCache_StopStopsCleanupWorker` asserts on `runtime.NumGoroutine()` deltas against a baseline taken at test start, so ANY neighbouring test whose teardown goroutines are still unwinding makes it fail. Observed failing once in a full-package run, then green on 3 consecutive full-package runs and 5 consecutive isolated runs; pre-existing (the file is untouched by wave 6) and the same family as the goroutine-count issues recorded under `FU-WAVE3-RELIABILITY`. Fix by asserting the cache's own worker exited (a done channel or `sync.WaitGroup` the cache exposes for tests) instead of a process-wide goroutine count. ~0.25d

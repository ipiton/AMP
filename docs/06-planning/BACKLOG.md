...
- [x] **FU-SLACK-PAGERDUTY-QUEUE-PATH** _(closed by wave 5, 2026-08-19)_ — the routing (`createPublisherForJob` → `CreatePublisherForTarget` for slack/pagerduty) and the `clientMu` guard over all four per-target client maps had already landed as part of the telegram fix (ed8c864, wave 3) — that commit covered rootly/pagerduty/slack/telegram/email in one shot, but this backlog line was never checked off and the slack/pagerduty-specific end-to-end proof was missing. Added the missing httptest coverage mirroring the telegram tests: queue-path job → `EnhancedSlackPublisher`/`EnhancedPagerDutyPublisher`, a real POST to the Slack webhook path and to PagerDuty's `/v2/events`, and the existing credential-missing-degrades-gracefully / concurrent-race coverage extended to both. No wiring code changed. ~0.25d (verification + tests only)
- [ ] **FU-SLACK-BUILDMESSAGE-TYPE-MISMATCH** — discovered while writing the above: `formatSlack` (formatter.go) returns `blocks`/`attachments` as `[]map[string]any`, but `EnhancedSlackPublisher.buildMessage`'s type assertions expect `[]interface{}` — a different slice type, so they never match. Every enhanced Slack notification currently posts an empty message body (no text, no blocks, no attachments); Slack itself may reject it. Out of FU5-A's scope (payload shape, not routing) — not fixed here. ~0.25d

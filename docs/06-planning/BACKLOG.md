...
- [x] **FU-FILE-SECRETS** _(opened + closed by wave 7 track B, 2026-08-19)_ — `*_file` secret variants for integration configs: `url_file` (webhook), `routing_key_file` (pagerduty), `api_url_file` (slack), `bot_token_file` (telegram), `slack_api_url_file` / `smtp_auth_password_file` (global) — the Kubernetes-Secret-volume shape upstream Alertmanager accepts for every credential field. `internal/infrastructure/routing.resolveFileSecrets` (new, `file_secrets.go`) enforces upstream's mutual-exclusion rule (inline XOR file, both = load error naming the field; unreadable file = load error naming the path) and resolves the file's content — trimmed of trailing newline/whitespace — into the SAME inline field `resolveGlobalFallbacks`, `applyDefaults`, validation and `business/publishing.BuildConfigTargets` already read, so the target builder needed zero changes. Re-read on every config load/reload (not lazily per-notification — documented divergence from upstream, requires `/-/reload`/SIGHUP for rotation). Status-API redaction (`internal/application/handlers/status_config.go`) taught a `fileSecretRefKeys` exception list so the `*_file` field's path stays visible (several of the new key names would otherwise trip an unrelated substring by coincidence — `routing_key_file` contains `routing_key`, `bot_token_file` contains `token` — and get redacted right along with the content they name) while the resolved secret stays redacted regardless of source. `global.smtp_auth_secret_file` (upstream's CRAM-MD5 secret) deliberately not added — AMP's SMTP publisher has no `smtp_auth_secret` field to pair it with. ~1d

...
- [x] **FU-PER-ALERT-OUTCOMES** _(opened + closed by wave 4, 2026-08-19)_ — per-alert outcome tracking for non-batch publishers (the wave-3 M4 residual): Slack/Telegram/PagerDuty/Email loop `Publish` per alert inside one job, so alert 3 of 5 failing left the whole `(group, target)` unconfirmed and the next fire re-sent all five, duplicating the four that had landed. Jobs now report which alerts the target accepted (`GroupPublishHandle.DeliveredAlerts`, readable mid-flight), the chain stores them as a per-`(group, target)` delivered set (`nflog:delivered:*`, TTL = repeat_interval + grace, capped at 500, cross-replica) and narrows the retry fire to the alerts still owed; in-job retries skip already-accepted alerts too. Keyed by `core.Alert.DeliveryKey` (`fingerprint:status`), fails open in the resend direction, batch targets untouched. ~1d

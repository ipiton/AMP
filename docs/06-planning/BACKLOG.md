...
- [ ] **FU-FLAKE-DUPLICATE-METRIC-KEYS** _(NOTE: already fixed on `main` by a wave-6 merge — this line survives only on branches based on 20a9d02, and the flake disappears once they merge)_ — `internal/business/publishing.TestEdgeCase_DuplicateMetricKeys` is a genuine concurrency race in the TEST's expectation, not an ordering issue: `PublishingMetricsCollector.CollectAll` runs collectors in goroutines, so "last writer wins" is whichever goroutine finishes last, and pinning a specific winner (200.0) fails whenever the other one lands second (concurrent CollectAll vs "last writer wins" assertion), pre-existing, documented across waves 2-5 gates. Fix the test's determinism. ~0.25d

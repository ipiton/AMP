...
- **AMP-PARITY-WAVE5** (2026-08-19): wave-5 tail items — publishing queue-path verification plus wave-4 delivered-state hardening (`fu4-out-review.md` r2/r5/r6).
  - **Slack/PagerDuty queue-path coverage** (`FU-SLACK-PAGERDUTY-QUEUE-PATH`): the routing itself (`PublishingQueue.createPublisherForJob` → `CreatePublisherForTarget` for rootly/pagerduty/slack/telegram/email, and the `clientMu` guard over all four per-target client caches) had already shipped as part of the wave-3 telegram fix — that commit covered every integration type in one pass, but the backlog line was never checked off and slack/pagerduty had no end-to-end proof of their own. Added httptest coverage mirroring the existing telegram tests: a queue-path job for a Slack/PagerDuty target now builds `EnhancedSlackPublisher`/`EnhancedPagerDutyPublisher` and is proven to POST to the real Slack webhook path and to PagerDuty's Events API v2 `/v2/events` (carrying the target's `routing_key`), plus the missing-credentials-degrades-gracefully case for both. No wiring code changed.
  - **Sliding delivered-state TTL fixed** (fu4-out-review r5): `RecordPartialDelivery` (both the Redis Lua script and the in-memory implementation) now sets the delivered state's TTL only on the write that CREATES the state, not on every subsequent partial write. Previously every fire that recorded even one new fingerprint refreshed the whole state's expiry, so a group with one persistently-failing alert and other alerts trickling in could keep already-delivered alerts suppressed well past one `repeat_interval`. The fix keeps the at-least-once floor (an under-refreshed TTL only ever expires *earlier*, i.e. resends sooner, never suppresses longer) and needs no schema change — Redis gates the `PEXPIRE` on `EXISTS` inside the same atomic script, the in-memory twin only stamps `recordedAt`/`ttl` when the state map didn't already exist.

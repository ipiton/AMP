...
func (p *nonDeliveringPublisher) PublishGroup(_ context.Context, _ string, alerts []*core.Alert, _ string, _ map[string]string, _ func(string, []*core.Alert) []*core.Alert) ([]TargetPublishOutcome, error) {
	if len(alerts) == 0 {
		return nil, nil
	}
	p.calls++

...
- **Допущение A1:** `rehydrateAlertStore` вызывается после `initializeInfrastructure`, то есть провайдер стора уже зарегистрирован. Проверяется в S4; если порядок другой — провайдер переносится в точку до рестора.
